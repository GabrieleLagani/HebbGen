...
class HebbianConv2d(nn.Module):
	...
	def __init__(self, in_channels, out_channels, kernel_size, stride=1,
		...
		self.register_buffer('delta_b', torch.zeros_like(self.bias))
	...
	def grad_e(self, y, t):
		#r = -((y * self.k).softmax(dim=1) - ((t if t is not None else 0.) * self.k).softmax(dim=1))
	...
	def compute_update(self, x, y, t=None, mult=1):
		...
		if self.mode == self.MODE_GRAD:
			with torch.no_grad():
				...
				r = self.grad_e(y, t).permute(1, 0, 2, 3)
		...
		if self.mode == self.MODE_SWTA:
			with torch.no_grad():
				...
				x_unf = F.unfold(x, kernel_size=self.kernel_size, stride=self.stride)
				x_unf = x_unf.permute(0, 2, 1).reshape(-1, x_unf.size(1))
				r = (y * (t if t is not None else 1.) * self.k).softmax(dim=1).permute(1, 0, 2, 3)
				r_sum = r.abs().sum(dim=(1, 2, 3), keepdim=True)
				r_sum = r_sum + (r_sum == 0).float()  # Prevent divisions by zero
				c = r.abs()/r_sum
				cr = c * r
				if self.patchwise:
					...
					self.delta_w += mult * (cr.reshape(r.shape[0], -1).matmul(x_unf) - dec).reshape_as(self.weight)
				else:
					...
					self.delta_w += mult * (cr.reshape(r.shape[0], -1).matmul(x_unf) - F.unfold(dec, kernel_size=self.kernel_size, stride=self.stride).sum(dim=-1)).reshape_as(self.weight)
		...
		if self.mode == self.MODE_HPCA:
			with torch.no_grad():
				...
				x_unf = F.unfold(x, kernel_size=self.kernel_size, stride=self.stride)
				x_unf = x_unf.permute(0, 2, 1).reshape(-1, x_unf.size(1))
				r =  (y * (t if t is not None else 1.)).permute(1, 0, 2, 3)
				...
				if self.patchwise:
					...
					self.delta_w += mult * (cr.permute(2, 3, 0, 1).reshape(r.shape[2], -1).matmul(x_unf) - F.unfold(dec, kernel_size=self.kernel_size, stride=self.stride).sum(dim=-1)).reshape_as(self.weight)
		...
		if self.mode == self.MODE_CONTRASTIVE:
			...
			L = - y_unf.sum(-1).reshape(-1, y.size(1)) * t.permute(0, 2, 3, 1).reshape(-1, t.size(1))
			...
			L = L + self.contrast * y_unf[idx].sum(-1).reshape(-1, y.size(1)) * t.permute(0, 2, 3, 1).reshape(-1, t.size(1))
	...
	def local_update(self):
		...
		self.delta_w.zero_()
		if self.bias.grad is None: self.bias.grad = -self.alpha * self.delta_b
		else: self.bias.grad = (1 - self.alpha) * self.bias.grad - self.alpha * self.delta_b
		self.delta_b.zero_()
...
class HebbianConvTranspose2d(HebbianConv2d):
	...
	def __init__(self, in_channels, out_channels, kernel_size, stride=1, w_nrm=True, bias=False, act=nn.Identity(),
		...
		super().__init__(in_channels, out_channels, kernel_size, stride, w_nrm, bias, act, mode, k, patchwise, contrast, uniformity, alpha)
		with torch.no_grad():
	...
	def compute_update(self, x, y, t=None, mult=1):
		...
		if self.mode == self.MODE_GRAD_T:
			with torch.no_grad():
				...
				c = r.abs()/r_sum
				cr = r #* c
				self.delta_w += mult * (cr.matmul(x.permute(0, 2, 3, 1).reshape(1, -1, x.size(1)))).permute(2, 1, 0).reshape_as(self.weight)
		
		if self.mode == self.MODE_SWTA_T:
			with torch.no_grad():
				# Logic for swta-type learning in transpose convolutional layers
				r = (y * (t if t is not None else 1.) * self.k).softmax(dim=1)
				r = F.unfold(r, kernel_size=self.kernel_size, stride=self.stride)
				r = r.permute(0, 2, 1).reshape(-1, self.out_channels, self.kernel_size[0]*self.kernel_size[1]).permute(2, 1, 0)
				r_sum = r.abs().sum(dim=-1, keepdim=True)
				r_sum = r_sum + (r_sum == 0).float()  # Prevent divisions by zero
				...
				self.delta_w += mult * (cr.matmul(x.permute(0, 2, 3, 1).reshape(1, -1, x.size(1))) - dec).permute(2, 1, 0).reshape_as(self.weight)
		
		if self.mode == self.MODE_HPCA_T:
			with torch.no_grad():
				# Logic for hpca-type learning in transpose convolutional layers
				r = y * (t if t is not None else 1.)
				r = F.unfold(r, kernel_size=self.kernel_size, stride=self.stride)
				r = r.permute(0, 2, 1).reshape(-1, self.out_channels, self.kernel_size[0]*self.kernel_size[1]).permute(2, 1, 0)
				c = 1/r.shape[2]
				cr = c * r
